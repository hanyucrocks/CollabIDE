...
2. Open the frontend, sign up, create a room. The badge should read
   **connected** — if it says _connecting_ forever, the WebSocket is failing;
...
reconnects with its new role. The _close frame_ can be delayed by the platform's
...
The client asks _this_ API to run code; only the server talks to Judge0.
